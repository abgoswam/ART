...
# Decrese the number of turns from 10 to 4 to avoid OOM errors on a T4
MAX_TURNS = 4
...
async def main():
    ...
    model = art.TrainableModel(
        name="email-agent-1013",
        project="email-search-agent",
        base_model="Qwen/Qwen2.5-3B-Instruct",
    )

    print(f"GPU generation: {torch.cuda.get_device_properties(0).major}")

    model._internal_config = art.dev.InternalModelConfig(
        init_args=art.dev.InitArgs(
            max_seq_length=1024,  # Reduce from 8192 to 4096 or even 2048
        ),
