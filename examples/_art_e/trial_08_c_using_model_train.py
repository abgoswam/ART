...
def env_setup():
    ...
    print("Is this uv managed?", ".venv" in sys.executable or "uv" in sys.executable)
...
# Decrese the number of turns from 10 to 4 to speed up training
MAX_TURNS = 4
...
async def using_the_model(training_scenarios, model):
...
async def train(training_scenarios, model):
    ...
    for batch in training_iterator:
        if batch.step % 16 == 0:
            print("============== batch.step:{batch.step} sample ==========")
            await using_the_model(
...
async def main():
    ...
    model = art.TrainableModel(
        name="email-agent-1014-qwen-3B-a",
        project="email-search-agent",
        base_model="Qwen/Qwen2.5-3B-Instruct",
    )

    # ADD THIS: Configure model for memory management and disable sleep mode
    model._internal_config = art.dev.InternalModelConfig(
        init_args=art.dev.InitArgs(
            max_seq_length=1024,  # Reduce from default to save memory
        ),
        engine_args=art.dev.EngineArgs(
            enforce_eager=True,
            gpu_memory_utilization=0.4,
            enable_sleep_mode=False,  # This fixes the sleep mode error
        ),
    )
    ...
    # Train
    await train(training_scenarios, model)
